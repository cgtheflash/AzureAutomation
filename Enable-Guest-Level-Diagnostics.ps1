    ...
    if ($ResourceType -eq "Microsoft.Compute/virtualMachines") {
      ...
      If ($OSType -eq 'Linux') {
        ...
        If ($($Extension.name) -notcontains 'LinuxDiagnostic') {
          ...
          Set-AzVMExtension -ResourceGroupName $ResourceGroupName -VMName $ResourceName -Location $VM.Location -ExtensionType LinuxDiagnostic -Publisher Microsoft.Azure.Diagnostics -Name LinuxDiagnostic -SettingString $publicSettings -ProtectedSettingString $protectedSettings -TypeHandlerVersion 4.0 
